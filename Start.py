...
class App(tk.Tk):
    ...
    def launch_analysis(self):
        ...
        try:
            ...
            run_compare_and_comment_ps(cut_docx_path, revised_docx_path, csv_path, out_docx_path)
...
def _build_prompt_preview(self, rel_docx: str, selected_labels: list, mode: str) -> str:
    ...
    return (f"Contexte. Tu dois réaliser une relecture experte du document suivant : {chemin_decoupee}. "
            ...
            f"Mise en évidence des modifications. Toutes les propositions de réécriture et ajouts de l’agent IA doivent être clairement signalés par un surlignage vert (green text highlight), en plus du suivi des modifications Word. "
            f"Utilise la mention 'Proposition de reformulation:' dans les commentaires Word pour introduire chaque texte remplacé ou ajouté, afin d’en faciliter la validation visuelle.\n\n"

            f"Modes d’intervention. Tu n’utilises que deux modalités: (1) modifications directes dans le corps du texte en suivi des modifications (les insertions seront surlignées en vert pour la lisibilité); (2) commentaires Word ancrés au plus près du passage concerné (comme un relecteur humain) pour justifier, signaler une limite, ou proposer une alternative lorsque l’édition directe n’est pas appropriée. "
            f"N’insère jamais de pseudo-commentaires sous forme de texte dans le corps du document (pas de [Commentaire] en ligne); réserve toute remarque au système de commentaires Word.\n\n"
